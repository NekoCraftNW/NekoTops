...
public final class NekoTops extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        this.hologramManager = new HologramManager(topManager);

        this.topUpdaterTask = new TopUpdaterTask(this);
        this.topUpdaterTask.run();
        long minutes = 1L;
        long time = minutes * 60 * 20L;
        topUpdaterTask.runTaskTimerAsynchronously(this, time, time);

        Bukkit.getOnlinePlayers().forEach(player ->
                this.hologramManager.getTops().forEach((s, topHologram) -> topHologram.loadHologram(player))
        );
    ...
    public void onDisable() {
        HologramPacketAdapter.getHolograms().forEach((player, holograms) -> {
            holograms.removeIf(hologram -> {
                if (this.topManager.getTops().get(hologram.getName()) != null) {
    ...
    public void log(String s) {
        Bukkit.getConsoleSender().sendMessage(MessagesUtils.colorize("&bNekoTops &8> &7" + s));
    }
}